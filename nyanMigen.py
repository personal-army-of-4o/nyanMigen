...
class nyanMigen:
    ...
    def _convert_assign(code, ctx):
        myslice = None
        (domain, target, value, myslice) = i = nyanMigen._parse_assign(code, None, [])
        ...
        if nyanMigen._can_convert_assign(i, ctx):
            ...
            if myslice:
                nyanMigen._parse_deps(myslice, ctx)
            nyanMigen._add_target(target, ctx)
            return nyanMigen._dump_assign(module, domain, target, value, myslice)
        else:
            raise Exception()
